...
/**
 * Extended options for our specialized depth of field effect
 */
...
const DOF_PRESETS: Record<DepthOfFieldPreset, ExtendedDepthOfFieldOptions> = {
  ...
  [DepthOfFieldPreset.MACRO]: {
    ...
    focalLength: 200,
    aperture: 1.4, // Instead of fStop
    focalDistance: 500, // Matching interface name
    focalDepth: 20,
    maxBlurStrength: 100
  },
...
export class DepthOfFieldEffect {
  ...
  private async transitionSettings(
    ...
    return new Promise<void>((resolve) => {
      ...
      const observer = this.scene.onBeforeRenderObservable.add(() => {
        ...
        dof.focalLength = this.lerp(fromSettings.focalLength || 50, toSettings.focalLength || 50, progress);
        
        // Use aperture instead of fStop
        if (fromSettings.aperture && toSettings.aperture) {
          dof.aperture = this.lerp(fromSettings.aperture, toSettings.aperture, progress);
        }
        
        // Handle focal distance property (maps to focusDistance in our extended options)
        ...
        dof.focalDistance = this.lerp(fromFocalDist, toFocalDist, progress);
        
        // Interpolate blur strength (kernel size)
        if (fromSettings.maxBlurStrength !== undefined && toSettings.maxBlurStrength !== undefined) {
          ...
          dof.kernelSize = this.mapBlurStrengthToKernelSize(currentBlur);
        } else if (fromSettings.blurAmount !== undefined && toSettings.blurAmount !== undefined) {
          // Or use direct blur amount if available
          dof.blur = this.lerp(fromSettings.blurAmount, toSettings.blurAmount, progress);
        }
  ...
  public setAutoFocus(enabled: boolean): void {
    ...
    if (enabled && this.focusTarget && this.pipeline.depthOfField) {
      this.autoFocusObserver = this.scene.onBeforeRenderObservable.add(() => {
        ...
        const currentFocusDistance = this.pipeline.depthOfField.focalDistance;
        const smoothingFactor = 0.05; // Adjust for slower/faster focus changes
        
        this.pipeline.depthOfField.focalDistance = 
  ...
  public async focusOnPosition(
    ...
    // Update focus distance with transition (using focalDistance to match BabylonJS API)
  ...
  public async rackFocus(
    ...
    const currentBlur = this.customSettings.maxBlurStrength || 
                        DOF_PRESETS[this.currentPreset].maxBlurStrength || 30;
    const increasedBlur = Math.min(currentBlur * 1.5, 100);
    
    // Apply initial focus (using focalDistance to match BabylonJS API)
    await this.updateSettings({ 
      focalDistance: fromDistance,
      maxBlurStrength: increasedBlur
    }, 0);
    
    // Transition to new focus
    await this.updateSettings({ 
      focalDistance: toDistance 
    }, duration);
    ...
    return new Promise<void>((resolve) => {
      const observer = this.scene.onBeforeRenderObservable.add(() => {
        ...
        if (this.pipeline.depthOfField) {
          ...
          this.pipeline.depthOfField.aperture = 
            (originalSettings.aperture || 2.8) * (1 - breathFactor * 0.15);
        }
